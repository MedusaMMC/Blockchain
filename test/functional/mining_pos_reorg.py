...
class ReorgStakeTest(PivxTestFramework):
    ...
    def check_money_supply(self, expected_piv, expected_zpiv):
        ...
        # verify that nodes have the expected MMC and zMMC supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_piv))
            for denom in node['zMMCsupply']:
                assert_equal(node['zMMCsupply'][denom], DecimalAmt(expected_zpiv[denom]))
    ...
    def run_test(self):
        ...
        # Check MMC and zMMC supply at the beginning
        # ------------------------------------------
        # zMMC supply: 2 coins for each denomination
        ...
        # MMC supply: block rewards minus burned fees for minting
        ...
        # Verify that MMC and zMMC supplies were properly updated after the spends and reorgs
        self.log.info("Check MMC and zMMC supply...")
