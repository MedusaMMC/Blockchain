...
void PIVXGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("Medusa client") + " " + networkStyle->getTitleAddText();
...
{
    QString strTitle =  tr("Medusa"); // default title
...
bool PIVXGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("Medusa"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
