...
const char * const PIVX_CONF_FILENAME = "medusa.conf";
const char * const PIVX_PID_FILENAME = "medusa.pid";
const char * const PIVX_MASTERNODE_CONF_FILENAME = "masternode.conf";


// Medusa only features
// Masternode
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\medusa
// Windows >= Vista: C:\Users\Username\AppData\Roaming\medusa
// Mac: ~/Library/Application Support/medusa
// Unix: ~/.medusa
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Medusa";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\medusaParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\medusaParams
    // Mac: ~/Library/Application Support/medusaParams
    // Unix: ~/.medusa-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MedusaParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "MedusaParams";
#else
    // Unix
    return pathRet / ".medusa-params";
#endif
#endif
}
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty medusa.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override medusa.conf
