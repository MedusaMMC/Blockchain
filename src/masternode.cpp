...
CAmount CMasternode::GetBlockValue(int nHeight)
{
    ...
    if(nHeight > 1) return 2.1 * COIN;
    if(nHeight > 0) return 210000 * COIN;
...
{
    if(nHeight <= 10) return 0;

    return CMasternode::GetBlockValue(nHeight) * 60 / 100;
}
...
bool CMasternodeBroadcast::CheckInputsAndAdd(int& nDoS)
{
    ...
    // should be at least not earlier than block when 1000 MMC tx got MASTERNODE_MIN_CONFIRMATIONS
    uint256 hashBlock = UINT256_ZERO;
    CTransaction tx2;
    GetTransaction(vin.prevout.hash, tx2, hashBlock, true);
    BlockMap::iterator mi = mapBlockIndex.find(hashBlock);
    if (mi != mapBlockIndex.end() && (*mi).second) {
        CBlockIndex* pMNIndex = (*mi).second;                                                        // block for 1000 MMC tx -> 1 confirmation
